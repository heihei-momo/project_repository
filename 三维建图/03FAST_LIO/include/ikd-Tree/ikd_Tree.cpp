...
int KD_TREE<PointType>::Delete_Point_Boxes(vector<BoxPointType> &BoxPoints)
{
    ...
    for (int i = 0; i < BoxPoints.size(); i++)
    {
        if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node)
        {
            tmp_counter/*这一批删除操作总共删除的点数*/ += Delete_by_range(&Root_Node, BoxPoints[i], true, false);//Root_Node是KD树的最根节点指针的指针
        }
        else
        {
            Operation_Logger_Type operation;
            ...
            pthread_mutex_lock(&working_flag_mutex);
            tmp_counter += Delete_by_range(&Root_Node, BoxPoints[i], false, false);//在根节点 Root_Node 上删除第 i 个盒子 BoxPoints[i]
            if (rebuild_flag)
            {
                pthread_mutex_lock(&rebuild_logger_mutex_lock);
                Rebuild_Logger.push(operation);
    ...
    // printf("Delete_by_range[%d]:",tmp_counter);
    return tmp_counter;
}
...
void KD_TREE<PointType>::Rebuild(KD_TREE_NODE **root)
{
    ...
    if ((*root)->TreeSize >= Multi_Thread_Rebuild_Point_Num)//判断是否使用多线程增量重建（子树足够大），这里 并没有真正重建树，只是标记了 Rebuild_Ptr，等待后台线程去重建
    {
    ...
    else
    {
        ...
        PCL_Storage.clear();
        flatten(*root, PCL_Storage, DELETE_POINTS_REC);//把当前节点及子树中所有有效点收集到一个线性数组 PCL_Storage
        delete_tree_nodes(root);//删除旧子树
        BuildTree(root, 0, PCL_Storage.size() - 1, PCL_Storage);
        if (*root != nullptr)
            (*root)->father_ptr = father_ptr;//保证新树仍然挂在原来的父节点上。
        if (*root == Root_Node)
...
{
    //支持懒删除和可选下采样的递归 KD-Tree 删除函数（只是标记删除），通过空间裁剪快速定位，支持多线程安全，必要时触发子树重建，以保证 KD-Tree 的平衡和高效搜索
    if ((*root) == nullptr || (*root)->tree_deleted)//空树或整棵子树已删除，直接返回
        return 0;
    (*root)->working_flag = true;
    Push_Down(*root);//把父节点的“删除/下采样标记”向左右子节点传播，单次调用只处理一层
    int tmp_counter = 0;
    ...
    {
        //如果 boxpoint 完全覆盖当前节点的包围盒 → 标记整棵子树删除
    ...
    if ((Rebuild_Ptr == nullptr) || (*root)->left_son_ptr != *Rebuild_Ptr)//当前没有子树在重建，或者左子树没有在重建
    {
    ...
    // 统计树大小和删除信息
    // TreeSize：节点及其子树总点数
    // invalid_point_num：被删除的点数量（懒删除标记）
    // down_del_num：被下采样删除的点数量
    // tree_deleted / tree_downsample_deleted：子树是否整体被删除/下采样
    // 更新节点包围盒（bounding box）和半径
    // node_range_x/y/z → 节点及子树在 xyz 方向的范围
    // radius_sq → 半边长平方和，用于快速搜索优化
    // 维护父指针
    // 确保左右子节点指向正确的父节点
    if (Rebuild_Ptr != nullptr && *Rebuild_Ptr == *root && (*root)->TreeSize < Multi_Thread_Rebuild_Point_Num)
        Rebuild_Ptr = nullptr;
    bool need_rebuild = allow_rebuild & Criterion_Check((*root))/*判断kd树是否要重建*/;
    //清理：如果子树里懒删除的点太多，重建可以移除它们。
    //重平衡：如果子树左右不平衡太严重，重建可以恢复 kd-tree 的搜索效率。
    if (need_rebuild)
        Rebuild(root);
    if ((*root) != nullptr)
        (*root)->working_flag = false;
